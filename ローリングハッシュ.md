...
// `impl IntoIterator<Item = u64>` のハッシュ化
...
pub mod rolling_hash {
  ...
  impl<'a, B: RollingHashBase> RollingHash<'a, B> {
    pub fn raw(base: &'a B, value: B::Value, len: usize) -> Self {
      Self { base, value, len }
    }
    ...
    pub fn repeat(self, times: usize) -> Self {
      ...
      Self::raw(self.base, value, self.len * times)
    }
  }
  ...
  pub trait RollingHashBase: Sized {
    ...
    fn new() -> Self;
    fn hash<'a>(&'a self, it: impl IntoIterator<Item = u64>) -> RollingHash<'a, Self>;
    ...
    fn hash_str(&self, str: &str) -> RollingHash<'_, Self> {
      self.hash(str.chars().map(u64::from))
    }
  }
    ...
    impl RollingHashBase for SingleBase {
      ...
      fn hash<'a>(&'a self, it: impl IntoIterator<Item = u64>) -> RollingHash<'a, Self> {
        ...
        for x in it {
          ...
          value = self.add(value, x + 1);
          len += 1;
        }
        RollingHash::raw(&self, value, len)
      }
