...
```rust
use persistent_avl_tree::*;
pub mod persistent_avl_tree {
    // Update: 2022-11-02 04:24
    ...
    impl<H: PersistentAVLTreeHelper> PersistentAVLTreeList<H> {
        /// 新しいリストを作成
        pub fn new() -> Self {
            Self { root: None }
        }
        ...
        pub fn merge(&self, other: &Self) -> Self {
            if let Some(left) = &self.root {
                Self { root: Some(Node::merge(self.back().unwrap().clone(), &left.pop_back(), &other.root)) }
            } else {
                other.clone()
            }
        }
        ...
        pub fn split(&self, idx: usize) -> (Self, Self) {
            ...
            if let Some(root) = self.root() {
                ...
                (Self { root: front }, Self { root: back })
            } else {
                (Self::new(), Self::new())
            }
        }
        ...
        pub fn reverse(&mut self) {
            self.root = self.root().map(|root| root.reverse(0, self.len()));
        }

        /// 区間 `range` の要素を逆順に並び替える O(logN)
        pub fn range_reverse(&mut self, range: impl RangeBounds<usize>) {
            let (l, r) = self.range(range);
            assert!(l <= r && r <= self.len());
            self.root = self.root().map(|root| root.reverse(l, r));
        }
    ...
    impl<H: PersistentAVLTreeHelper> Clone for PersistentAVLTreeList<H> {
        fn clone(&self) -> Self {
            Self { root: self.root.clone() }
        }
    }
    impl<H: PersistentAVLTreeHelper> Index<usize> for PersistentAVLTreeList<H> {
        type Output = H::S;
    ...
    impl<H: PersistentAVLTreeHelper> Debug for PersistentAVLTreeList<H> where H::S: Debug {
    ...
    struct Node<H: PersistentAVLTreeHelper>(Rc<UnsafeCell<RawNode<H>>>);
    impl<H: PersistentAVLTreeHelper> Node<H> {
        pub fn new(value: H::S) -> Self {
            Self(Rc::new(UnsafeCell::new(RawNode::new(value))))
        }
        ...
        pub fn reverse(&self, mut l: usize, mut r: usize) -> Self {
            self.get_mut().push();
            let mut this = Self::new(self.value.clone());
            if l == r {
                this.child = self.child.clone();
            ...
            if let Some(left) = &self.child[L] {
                this.child[L] = Some(left.reverse(l, r.min(left.len)));
                l = l.saturating_sub(left.len);
                r = r.saturating_sub(left.len);
            }
            if l == 0 && r >= 1 {
                this.value = H::rev(&this.value);
                l = l.saturating_sub(1);
                r = r.saturating_sub(1);
            }
            if let Some(right) = &self.child[R] {
                this.child[R] = Some(right.reverse(l, r));
            }
            this.update();
            this
        }
        ...
        pub fn pop_front(&self) -> Option<Self> {
            self.get_mut().push();
            if let Some(left) = &self.child[L] {
                Some(Self::merge(self.value.clone(), &left.pop_back(), &self.child[R]))
            } else {
                self.child[L].clone()
            }
        }
        ...
        fn build(x: H::S, l: Option<Self>, r: Option<Self>) -> Self {
    ...
    struct RawNode<H: PersistentAVLTreeHelper> {
        pub height: isize,
        pub len: usize,
        pub rev: bool,
        pub value: H::S,
