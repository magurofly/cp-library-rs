...
pub mod persistent_avl_tree {
    ...
    impl<H: PersistentAVLTreeHelper> From<Vec<H::S>> for PersistentAVLTreeList<H> {
    ...
    impl<H: PersistentAVLTreeHelper> From<Option<Node<H>>> for PersistentAVLTreeList<H> {
        fn from(root: Option<Node<H>>) -> Self {
            Self { root }
        }
    }
    ...
    impl<H: PersistentAVLTreeHelper> Node<H> {
        ...
        pub fn merge(root_value: H::S, left: &Option<Self>, right: &Option<Self>) -> Self {
            eprintln!("merge(root, n={}, n={}): bias={}", left.as_ref().map(Node::len).unwrap_or(0), right.as_ref().map(Node::len).unwrap_or(0), Self::bias(left, right));
