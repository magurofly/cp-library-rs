...
```rust
fn dijkstra<E, C: Copy + Num + Eq + Ord>(graph: &Vec<Vec<(usize, E>>, start: usize, cost: impl FnMut(&(usize, E), C) -> Option<C>) -> Vec<Option<C>>;
```

- `graph`: 隣接リスト形式の（有向）グラフであり、辺の情報は `(to: usize, weight: E)` の形式
- `start`: 始点の頂点番号
- `cost`: 距離計算関数であり、辺と現在の距離を渡すと、 `Option<C>` を返す。
